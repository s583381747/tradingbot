"""
Chop-Box Trend-Following Strategy — Gen 2.

Gen 2 changes from Gen 0:
  - Add volume confirmation on bounce candle (放量突破)
  - Add wick-based pullback detection (only this fix from Gen 1)
  - Loosen chop threshold slightly (0.012→0.008) to get more trades
  - Keep daily chop reset (Gen 1 showed removing it hurts quality)
"""
...
CHOP_SLOPE_THRESHOLD = 0.008    # Gen 2: loosened from 0.012 to get more trades; volume filter compensates
...
# Volume Confirmation (Gen 2)
VOL_MA_PERIOD = 20              # volume moving average period
VOL_BREAKOUT_MULT = 1.5         # bounce candle volume must be > N x avg volume
...
class Strategy(bt.Strategy):
    ...
    def _run_entry_logic(self):
        ...
        # Gen 2: wick-based pullback detection
        if trend == 1:
        ...
        if self._pb_state == 0:
            ...
            if in_zone:
                ...
                # Gen 2: require above-average volume on bounce candle (放量突破)
                vol_ok = self.data.volume[0] > self.vol_ma[0] * self.p.vol_breakout_mult

                if on_side and self._pb_bar_count >= self.p.min_pullback_bars and momentum and vol_ok:
