"""
Chop-Box Trend-Following Strategy — Gen 4 (resistance breakout filter).

Gen 4: Add structural confirmation to entry. Before accepting a pullback entry:
  - Long: price must have recently broken above a swing high (structure confirms uptrend)
  - Short: price must have recently broken below a swing low
This filters out bear-market bounces where EMA pullback triggers but trend reverses.

Also widen stop to 5.0 ATR (Gen 3 finding on SIP data).
"""
...
# Resistance/Support Breakout Confirmation (Gen 4)
SWING_LOOKBACK = 30             # bars to look back for swing high/low
SWING_RECENCY = 60              # breakout must have happened within this many bars

# Initial Stop (below pullback extreme = EMA test candle low)
INITIAL_STOP_ATR_MULT = 5.0     # Gen 4: wide stop for SIP data
...
TP1_CANDLE_OFFSET = 1.00        # ← was 0.10; 1-min candles need wide buffer
TP2_EMA_ATR_MULT = 5.0          # ← was 3.0; wider trail = more profit captured
TP3_EMA_ATR_MULT = 8.0          # ← was 6.0; let winners run
...
class Strategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # Resistance breakout tracking (Gen 4)
        self._recent_swing_high = 0.0
        self._recent_swing_low = 999999.0
        self._broke_high_bar = -999  # bar index when swing high was broken
        self._broke_low_bar = -999
    ...
    # ─────────────────────────────────────── swing tracking (Gen 4)
    def _update_swing_levels(self):
        """Track swing high/low and detect breakouts."""
        lb = self.p.swing_lookback
        if len(self) < lb + 1:
            return

        # Find highest high and lowest low in lookback window (excluding current bar)
        swing_high = max(self.data.high.get(ago=-i, size=1)[0] for i in range(1, lb + 1))
        swing_low = min(self.data.low.get(ago=-i, size=1)[0] for i in range(1, lb + 1))

        # Detect breakout: current close breaks above swing high
        if self.data.close[0] > swing_high and swing_high > self._recent_swing_high * 0.999:
            self._broke_high_bar = len(self)
            self._recent_swing_high = swing_high

        # Detect breakdown: current close breaks below swing low
        if self.data.close[0] < swing_low and swing_low < self._recent_swing_low * 1.001:
            self._broke_low_bar = len(self)
            self._recent_swing_low = swing_low

    def _has_breakout(self, direction: int) -> bool:
        """Check if a recent structural breakout occurred in the given direction."""
        recency = self.p.swing_recency
        if direction == 1:
            return (len(self) - self._broke_high_bar) <= recency
        else:
            return (len(self) - self._broke_low_bar) <= recency
    ...
    def _run_entry_logic(self):
        ...
        if self._pb_state == 0:
            ...
            if in_zone:
                ...
                # Gen 4: require recent structural breakout
                breakout_ok = self._has_breakout(trend)

                if on_side and self._pb_bar_count >= self.p.min_pullback_bars and momentum and breakout_ok:
