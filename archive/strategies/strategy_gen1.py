"""
Chop-Box Trend-Following Strategy v3 — Gen 1 (bug fixes).

Core: Detect consolidation boxes via EMA slope persistence. Only trade after
price breaks out of the box. Entry on EMA20 pullback bounce. Exit via 3-portion
cascading trailing stops (30%/40%/30%). Addon on EMA retest sized to breakeven.

v3 changes (data-driven from 200+ experiment configs):
  - EMA_SLOPE_PERIOD 5→3: PF 0.66→1.41 (faster trend detection)
  - CHOP_SLOPE_THRESHOLD 0.005→0.012: filters 80%+ bad trades
  - INITIAL_STOP_ATR_MULT 1.5→2.0: optimal stop width
  - TP_ACTIVATE_ATR 2.0→5.0: wait for real trend confirmation
  - TP1_CANDLE_OFFSET 0.10→1.00: 10x more breathing room
  - TP2/TP3 trails widened: 3/6→5/8 ATR (preserves more upside)
  - LOSERS_MAX_BARS 60→45: cut losers faster
  - MAX_ADDONS 2→1: more conservative pyramiding
"""
...
INITIAL_STOP_ATR_MULT = 2.0     # ← was 1.5; 2.0 optimal (PF 1.02→1.51)
...
class Strategy(bt.Strategy):
    ...
    def _check_new_day(self):
        ...
        if dt != self._today:
            ...
            # Chop box, slope history, pullback state persist across days
    ...
    def _run_entry_logic(self):
        ...
        if trend == 0:
            # Bug fix: tolerate short flicker before resetting pullback
            self._trend_zero_count += 1
            if self._trend_zero_count > self.p.trend_flicker_bars:
                self._pb_state = 0
                self._pb_bar_count = 0
            return

        self._trend_zero_count = 0

        # Reset on trend flip (actual direction change)
        ...
        # Bug fix: use wick (Low/High) for pullback touch, not Close
        if trend == 1:
            touch = self.data.low[0] <= self.ema[0] + atr_val * self.p.pullback_touch_mult
            not_through = self.data.low[0] >= self.ema[0] - atr_val * self.p.pullback_touch_mult
            in_zone = touch and not_through
        else:
            touch = self.data.high[0] >= self.ema[0] - atr_val * self.p.pullback_touch_mult
            not_through = self.data.high[0] <= self.ema[0] + atr_val * self.p.pullback_touch_mult
            in_zone = touch and not_through
