"""
Chop-Box Trend-Following Strategy — Gen 6 (price-based chop box).

STRUCTURAL FIX: Chop box detection rewritten from EMA slope → price range compression.

Old method (Gen 0-5): EMA slope < threshold → chop (82% of bars = chop on SIP data!)
New method (Gen 6):   N-bar price range / ATR < threshold → chop (~15-25% = chop)

This is the correct interpretation of "chop box":
  - Track actual price high/low over N bars
  - If range is narrow relative to ATR → consolidation (box)
  - When price breaks out of box → trend starting → look for EMA pullback

Also: trend detection simplified to EMA alignment (price > EMA20 > EMA50).
No more EMA slope threshold for trend — just structural alignment.
"""
...
class Strategy(bt.Strategy):
    ...
    def _update_chop_box(self):
        """Price-range chop detection. Chop = N-bar range / ATR < threshold."""
        atr_val = self.atr[0]
        ...
        if self._in_chop:
            ...
            # Breakout: range expanding AND price outside box
            if range_ratio >= self.p.chop_range_atr_ratio and self._chop_bar_count >= self.p.chop_box_min_bars:
                self._in_chop = False
        else:
            # In trend: check if range compresses again → new chop
    ...
    def _detect_trend(self) -> int:
        """Simple EMA alignment trend detection. No slope threshold."""
