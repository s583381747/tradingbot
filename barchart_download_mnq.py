"""
Download MNQ (Micro E-mini Nasdaq 100 Futures) 1-minute data from Barchart.
4 years: 2022-03-26 to 2026-03-26.

Uses Playwright with its OWN isolated profile + injected cookies from Chrome.
Does NOT touch Chrome browser or its profile in any way.
"""
import asyncio
...
DATA_DIR = Path("data/barchart_mnq")
DATA_DIR.mkdir(parents=True, exist_ok=True)

URL = "https://www.barchart.com/futures/quotes/NM*0/historical-download"
...
def get_barchart_cookies():
    """Read-only extraction of cookies from Chrome's cookie DB."""
...
async def download_one_range(page, date_from, date_to, idx, total):
    ...
    filename = f"MNQ_1min_{date_from.replace('/', '-')}_{date_to.replace('/', '-')}.csv"
...
async def main():
    ...
    print(f"MNQ Futures 1-min data | {len(ranges)} ranges")
    print(f"Output: {DATA_DIR.resolve()}\n")

    pw_cookies = get_barchart_cookies()

    async with async_playwright() as p:
        # Launch Playwright's OWN Chromium — completely separate from Chrome
        ...
        # Inject cookies into Playwright's isolated context
        await context.add_cookies(pw_cookies)
        page = await context.new_page()

        print("Loading MNQ futures page (Playwright Chromium, not Chrome)...")
        ...
        try:
            ...
            if has_form == 0:
                ...
                print("Form NOT found. Page text:")
                for l in lines:
                    print(f"  {l[:100]}")
                await browser.close()
                return
